...
def log_with_timestamp(data: Any, log_file: str = 'logs/log.json') -> None:
    """
    Log data with a timestamp to a JSON log file.

    Args:
        data (Any): The data to be logged. Can be of any JSON-serializable type.
        log_file (str, optional): Path to the log file. Defaults to 'logs/log.json'.
    ...
    try:
        ...
        raise TypeError(f"Data cannot be JSON serialized: {e}")
    except IOError as e:
